...
def add_to_list(item, due_date):
    try:
        conn = sqlite3.connect(DB_PATH)

        c = conn.cursor()
        c.execute("insert into items(item, status, due_date) values (?,?,?)",(item, NOTSTARTED,due_date))
        conn.commit()
        return {"item": item, "status": NOTSTARTED, "due_date":due_date}
